...
export async function onRequestGet({ env, request, waitUntil }) {
  ...
  const edgeCache = async (key, ttl, produce) => {
    ...
    // 空っぽの結果はキャッシュしない（一時的な取得失敗で「写真ゼロ」が固定化するのを防ぐ）
  ...
  //  Cloudinary の Admin API には 1時間あたりの呼び出し上限があり、アクセスの
  //  たびに全画像を検索すると簡単に上限（HTTP 420）に達してしまう。
  //  そこで「母集団（画像一覧）」はエッジにキャッシュし、抽選だけを毎回行う。
